...
trait Import {
    ...
    public function import($class, $role, $options=[]): false | array
    {
        ...
        try {
            ...
            $options['import'] = true;
            set_time_limit(0);
            $object->config('difference.fun.node.import.start', microtime(true));
            ...
            if($data){
                ...
                $object->config('difference.fun.node.import.list.count', $list_count);
    ...
    private function update($class, $role, $options=[], $create_many=[], $put_many=[], $patch_many=[], $skip=0): array
    {
        ...
        if(!empty($error)){
            ...
            return [
                'error' => $error,
                'transaction' => true,
                'duration' => (microtime(true) - $object->config('difference.fun.node.import.start')) * 1000
            ];
        }
        $commit = [];
        ...
        $duration = microtime(true) - $object->config('difference.fun.node.import.start');
