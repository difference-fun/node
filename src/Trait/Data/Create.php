...
trait Create {
    ...
    public function create_many($class, $role, $nodeList=[], $options=[]): false | array
    {
                ...
                if ($validate->success === true) {
                    ...
                    if (
                        ...
                        if(
                            ...
                            Event::trigger($object, 'difference.fun.node.create', [
                                'class' => $name,
                                'node' => $record,
                                'options' => $options,
                                'role' => $role
                        ...
                        if($options['import'] === true){
                            $number = $object->config('difference.fun.node.import.list.number');
                            if(empty($number)){
                                $number = 1;
                            } else {
                                $number++;
                            }
                            $object->config('difference.fun.node.import.list.number', $number);
                            $amount = $object->config('difference.fun.node.import.list.count');
                            if($amount > 0){
                                if($number === 1){
        ...
        $object->config('delete', 'difference.fun.node.import.list');
        if(!empty($error)){
            $response = [];
