...
trait Relation {
        ...
        if($data){
            ...
            foreach($relations as $relation){
                if(
                    ...
                    /* done already in difference_fun/node nodelist 1575
                    if(
                        empty($output_filter) &&
                        property_exists($relation, 'output') &&
                        !empty($relation->output) &&
    ...
    public function relation_mtime($object_data): array
    {
        ...
        $loaded = $object->config('difference.fun.node.relation.mtime.loaded');
        $is_outer = false;
        if(!$loaded){
            $is_outer = true;
            $loaded = [];
        ...
        if(
            ...
            foreach($relations as $relation){
                ...
                $object->config('difference.fun.node.relation.mtime.loaded', $loaded);
        ...
        $object->config('difference.fun.node.relation.mtime.loaded', $loaded);
        if($is_outer){
            $object->config('delete', 'difference.fun.node.relation.mtime.loaded');
        }
        return $mtime;
    }
}